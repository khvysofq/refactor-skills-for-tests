...
**执行顺序**：首轮执行 Task 1 → 1.5 → 2 → 2.5（条件） → 3 → 4 → 5；修正轮由分解审视驱动，可只执行 Task 2–4 中受影响的模块、Task 4（更新总报告）与 Task 5。
...
**目标**：按 [模块报告模板](../templates/module-report.md) 填写职责、边界、依赖、信息来源、使用示例
...
6. **填写「信息来源」**：记录分析所依据的信息源（任务 1.5 的产出）
7. **填写「使用示例」**：提供至少一个基本用法示例
