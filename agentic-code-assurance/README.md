...
> **读取指令**：本文档是 Agent（如 OpenCode、ClaudeCode）执行自动化 BUG 分析的总入口。先阅读本文档以理解整条流水线、各阶段输入输出与反馈机制，再根据当前目标进入对应子目录的工作流。
...
1. **先读本 README**，确认三阶段顺序、输入输出契约与反馈机制。
2. **根据当前目标与前置产出状态**，进入对应子目录：
...
| **2-risk-assessment/** | 风险分析工作流。入口：[Workflow.md](2-risk-assessment/Workflow.md)。输入：`docs/codearch/`；输出：疑似 BUG 任务列表。详细 Phases/Skills 后续补充。 |
| **3-bug-remediation/** | BUG 确认与修复工作流。入口：[Workflow.md](3-bug-remediation/Workflow.md)。输入：任务列表与知识库；输出：测试与修复产物。详细步骤后续补充。 |
