...
内容须覆盖：编译系统说明、主要构建命令、是否存在单元测试体系、如何运行测试（命令、环境、可选过滤）。
...
| 类别 | 说明 |
|---|---|
| **DO** | 以可执行命令为主，便于后续 Agent 或人工复现 |
| **DO** | 无单元测试时明确写出「当前无单元测试体系」 |
| **DON'T** | 不臆造不存在的 target 或命令 |
| **DON'T** | 不省略环境或前置步骤（若运行测试依赖先 build，需写明） |
