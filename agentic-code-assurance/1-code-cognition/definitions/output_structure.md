...
11. **并发不变量**（仅多线程模块必填）
    ...
    - 非多线程模块标注「本模块为单线程模型」
12. **与其它模块的关系**（文字或依赖方向说明，可选 Mermaid 图）
...
- **推荐用法**：进行风险分析或深度审查时，先加载 `overall_report.md` 查看技术特征概览，再根据改动涉及的文件/路径加载对应的 `modules/*.md`；利用「代码特征」和「关键代码位置索引」快速定位需要关注的代码区域；然后基于「关键数据流路径」「生命周期与所有权模型」「并发不变量」做路径级深度追踪。
