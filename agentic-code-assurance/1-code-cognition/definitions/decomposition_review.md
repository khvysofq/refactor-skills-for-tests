...
| **粒度一致**         | 是否有的模块过大（应考虑再拆）、有的过碎（应考虑合并）；与复杂度评级是否匹配；**极高复杂度模块是否已按 [拆分建议触发条件](complexity_levels.md) 评估拆分可行性，且报告中有明确说明（已拆/不拆的理由）** |
...
> **粒度一致的强制要求**：若存在极高复杂度模块，且其报告中无拆分说明（「已拆」或「不拆的理由」），则此维度直接不通过，须补充评估后重新审视。
