...
1. 阅读 `docs/codearch/overall_report.md` 的工程概览、主流程、模块索引（含模块列表、依赖图、跨模块关系摘要）、技术特征概览。
2. 注意模块列表中的「状态」列：「叶子」模块可直接分析，「已拆分」模块需查看其子模块（以 ↳ 缩进显示）。
3. 理解工程整体与各模块的复杂度、技术特征统计（如哪些模块涉及多线程、外部输入等）。
