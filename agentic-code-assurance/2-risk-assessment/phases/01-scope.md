...
> **前置条件**：从 [Workflow.md](../Workflow.md) 决策树 Q1 为「否」进入。  
...
- 从 [Workflow.md](../Workflow.md) 入口决策树判断 Q1 为「否」（分析范围与策略尚未确定）
- 或需重新划定分析范围
...
**加载 Skill**：阅读并执行 → [Skill 01: 范围与策略](../skills/skill-01-scope.md)

**按需查阅**：确定风险维度时阅读 [风险类型定义](../definitions/risk_types.md)
...
- [ ] **包含风险维度/优先级**：至少说明重点审查的风险类型（与 [risk_types](../definitions/risk_types.md) 对齐）
...
| 验收结果 | 下一步 |
|----------|--------|
| 通过 | 回到 [Workflow.md](../Workflow.md) 决策树：若 Q2 为「否」则进入 [Phase 02: 深度审查](02-review.md)；若 Q2 为「是」则继续 Q3 |
...
**执行**：立即加载 [Skill 01](../skills/skill-01-scope.md) 开始执行
